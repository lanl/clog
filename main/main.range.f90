...
!!        nn =300
!!        dt=tmax/(nts*nit)
!!        te=30.       ! Electron temperature       [keV]
!!        ti=30.       ! Ion temperature            [keV]
!!        ne=1.E25     ! Electron number density    [cm^-3]
!!        ep=3540.     ! Projectile energy  [keV]
!!        mp=4*MPKEV   ! Projectile mass    [keV]
!!        zp=2.        ! Projectile charge  [e]
...
!!        ne=1.E23     ! Electron number density    [cm^-3]
!!        ep=185000./2.! Projectile energy  [keV]
!!        mp=120*MPKEV ! Projectile mass    [keV]
!!        zp=25.       ! Projectile charge  [e]
!!        OPEN(1, FILE='range.003.out')
!!        OPEN(2, FILE='dedx.003.out')
...
!!        mp=120*MPKEV ! Projectile mass    [keV]
!!        zp=25.       ! Projectile charge  [e]
!!        OPEN(1, FILE='range.004.out')
!!        OPEN(2, FILE='dedx.004.out')

!
! DT plasma with alpha particle projectile
!
      CALL define_plasma(te,ti,ne,nni)
!!    CALL define_plasma_FF(te,ti,ne,nni)
!
...
! Allocates other plasma arrays
!
    SUBROUTINE define_plasma_FF(te, ti, ne, nni)
    USE allocatablevars
    USE physvars
      IMPLICIT NONE
      REAL                                :: te     ! [keV]
...
!
      ...
      betab(1)=1./te                    ! inverse temp array   [keV^-1]
      betab(2:nni+1)=1./ti              !
    END SUBROUTINE define_plasma_FF
